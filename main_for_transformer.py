...
try:
    ...
    for epoch in tqdm(range(1, args.epochs + 1), 'training:'):
        ...
        val_loss, val_rae, val_corr, val_rmse, val_mae = eval_method(Data, Data.valid[0], Data.valid[1], model,
                                                                     evaluateL2, evaluateL1, args)
        print(
            '| end of epoch {:3d} | time used: {:5.2f}s | train_loss {:5.4f} | valid rse {:5.4f} | valid rae {:5.4f} | valid corr  {:5.4f}| val RMSE {:5.4f}|val mae {:5.4f}'.
                format(epoch, (time.time() - epoch_start_time), train_loss, val_loss, val_rae, float(val_corr),
                       float(val_rmse), float(val_mae)))
        ...
        if val_loss < best_val:
            ...
            best_val = val_loss
        if epoch % 10 == 0:
            test_acc, test_rae, test_corr, test_rmse, test_mae = eval_method(Data, Data.test[0], Data.test[1], model,
                                                                             evaluateL2, evaluateL1, args)
            print("| test rse {:5.4f} | test rae {:5.4f} | test corr {:5.4f}| test rmse {:5.4f} | test mae \n".format(
                test_acc, test_rae,
                float(test_corr), float(test_rmse), float(test_mae)))
...
print(
    "| test rse {:5.4f} | test rae {:5.4f} | test corr {:5.4f} | test rmse {:5.4f} | test mae {:5.4f}".format(test_acc,
                                                                                                              test_rae,
                                                                                                              float(
                                                                                                                  test_corr),
                                                                                                              test_rmse,
